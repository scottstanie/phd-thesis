...
The full vector of LOS surface velocities between adjacent SAR acquisitions $ \bm{v} = \left[v_1 , \ldots , v_{N-1} \right]^T $ can be solved as:
\begin{equation}
...
For the time series methods which calculated an average velocity $v_{avg,j}$ over the $j$th time period $T_j$ , we computed the cumulative LOS surface deformation as the product $v_{avg,j} \cdot T_j$.
