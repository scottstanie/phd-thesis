...
%	v_{avg} = \frac{ \sum_{i \in G} \bm{\Delta  d}_i}{\sum_{i \in G} \bm{\Delta t}_i}
	v_{avg} = \frac{ \sum_{i \in G} \bm{\Delta  \phi}_i}{\sum_{i \in G} \bm{\Delta t}_i}
	\label{eq:ch4-stacking}
\end{equation}
where $G$ is a subset of interferograms that were formed using two SAR scenes acquired within the time period $T$. The LOS measurement (in cm) and the temporal baseline of the $i^{th}$ interferogram in $G$ are written as $  \bm{\Delta  \phi}_i $ and $ \bm{\Delta t}_i $ respectively. 

$ \bm{\Delta d}_i = \frac{4 \pi}{\lambda} \bm{\Delta  \phi}_i $
...
To compare the time series methods and the outlier removal strategy, we projected the GPS ENU time series onto the radar LOS at all 13 permanent GPS stations. We computed the average GPS LOS velocity with a linear regression and used this as ground truth to compare the InSAR solutions for ascending Path 78 data between Nov. 2014 and Jan. 2018.
We found that InSAR tropospheric noise has a strong influence on all the surface deformation solutions before removing the measurement outliers. As an example, Figure \ref{fig:ch4-compare-ts} (a) shows the ascending LOS solutions between Nov. 2014 and Jan. 2018 at the GPS station TXSO before removing InSAR measurement outliers. The random tropospheric noise creates up to $\sim$6 cm of error in the unregularized SBAS surface deformation time series. This error can be reduced by increasing $ \alpha$ in Equation \eqref{eq:ch4-sbas}. As $\alpha$ increases, the LOS deformation time series converge to the $L_2$ linear deformation (constant velocity) solution. 
...
After removing InSAR outlier measurements associated with local weather events, all SBAS time series methods produce more accurate and consistent deformation trends (Figure \ref{fig:ch4-compare-ts} (b)). The unregularized SBAS time series still contain up to $\sim$3 cm of tropospheric noise, which leads to long-wavelength artifacts in the basin-wide deformation maps. The $ L_1 $ and $ L_2 $ linear deformation solutions show close agreement ($<$ 2 mm difference) at all GPS stations. Since both methods suggest a deformation trend consistent with the stacking approach, we chose the simple stacking method as the final processing strategy for estimating cumulative surface deformation in the Permian Basin. 
Table \ref{tab:compare-errors} summarizes the differences before and after removing outliers for the four methods using the root mean square (RMS) and the worst-case absolute differences between InSAR and GPS inferred average LOS velocities.
% between Nov. 2014 and Jan. 2018.
% as derived from the four InSAR time series methods.
...
We performed the stacking and outlier removal algorithm on the Path 78

We see a striking visual difference in the Path 78 cumulative deformation from Nov. 2014 to Jan. 2018.derived from stacking after removing tropospheric outliers (Figure \ref{fig:ch4-outliers-visual}).
There are cm-level tropospheric artifacts in the cumulative deformation solutions by using either $L_1$ or $L_2$ linear deformation models (e.g. Figure \ref{fig:ch4-outliers-visual}(a)).   
...
These long wavelength artifacts (e.g. the red near Midland) in the cumulative deformation solutions did not match the GPS stations measurements.
However, the artifacts were mitigated in the stacking solutions through the pixel-wise outlier removal algorithm (Figure \ref{fig:ch4-outliers-visual} (b)). 
...
We summarize the errors in the InSAR stacking solutions before and after the outlier removal in Tables \ref{tab:gps-error-78} and \ref{tab:gps-error-85}. 
