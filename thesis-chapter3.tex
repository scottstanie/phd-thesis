...
The second example illustrates strong tropospheric noise resulting from a heat wave, which creates a $\sim 20$ cm decrease in LOS delay between moving the northwest to southeast portions of the interferogram (Figure \ref{fig:ch2-tropo-correct-wave}b). Note that this change is not visible in the optical GOES image (Figure \ref{fig:ch2-tropo-correct-wave}a), but is present in the GMAO weather model (Figure \ref{fig:ch2-tropo-correct-wave}c). Although the root mean square (RMS) noise of the corrected interferogram drops from 6.1 cm to 2.6 cm, the slight temporal differences of the model output and interferogram acquisition times lead to only a partial correction of the tropospheric pattern.
