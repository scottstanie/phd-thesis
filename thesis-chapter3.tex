...
%, and permanent GPS stations are too sparse to provide a full picture of surface changes.
...
\subsection{InSAR processing strategy}
...
We chose the GPS station TXKM as the reference point for both ascending and descending InSAR data, and we used the remaining 13 stations as controls to assess InSAR measurement uncertainty as described in Section \ref{sec:errors}


As outlined in Section \ref{sec:ch2-noise}, an interferogram may include noise from many possible sources.
%orbital errors, phase decorrelation, unwrapping errors, DEM inaccuracies, ionospheric and tropospheric artifacts, and other residual noise terms. 
In our dataset, $\Delta \phi_{orb}$, $\Delta \phi_{decor}$, and $\Delta \phi_{unwrap}$ were either removed or were negligible. Because the relatively flat Permian Basin is located in the middle latitudes, $\Delta \phi_{dem}$ and $\Delta \phi_{iono}$ are not substantial \citep{Fattahi2013DemErrorCorrection, Liang2019IonosphericCorrectionInsar}. 
...
The dominant noise term for our West Texas Sentinel-1 data was tropospheric noise; evaluation and mitigation strategies are presented in Chapter \ref{CHAP:4-GRL} and \ref{CHAP:5-robust-ts}.
