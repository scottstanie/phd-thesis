...
The first two terms, $ \Delta \phi_{orb} $ and $ \Delta \phi_{dem} $, represent systematic errors resulting from imprecise knowledge of the platform's orbital position or topographic height. Orbital errors usually appear as a planar phase ramp in stripmap acquisitions, and were commonly encountered for ERS-1 or ENVISAT data; however they rately appear in Sentinel-1 data due to it's high precision orbit determination \cite{Fattahi2014InsarUncertaintyDue}. Errors in the DEM result in an incorrect topograph phase removal and create a spatial baseline-dependent phase error \cite{Fattahi2013DemErrorCorrection}. These errors are generally in Sentinel-1 data due to the tight control of the repeat orbit tube.
...
Decorrelation errors, $ \Delta \phi_{decor} $, arise from a loss of coherence of the phase between $S_1$ and $S_2$ \cite{Zebker1992DecorrelationInterferometricRadar}. 
...
\end{equation}
where $ \left\langle \cdot \right\rangle  $ denotes an ensemble average, which, in practice, is estimated using an average of pixels over some spatial window.
...
In high signal to noise ratio (SNR) interferograms (e.g. for large magnitude events like the 1992 Landers, CA earthquake \cite{Massonnet1993DisplacementFieldLanders}), one can estimate the change in range caused by a deformation event by simply ``counting the fringes'' and multiplying by $\lambda/2$. However, it is more common for an interferogram to contain many visual features corresponding to noise terms in Equation \eqref{eq:noise-terms}. These noise sources can make interpretation difficult for inexperienced InSAR users, but they can often be visually identified.
% are as large or larger than 
%, this is often the exception, rather than the rule.
...
To illustrate this, Figure \ref{fig:ch3-sar-hawaii-example} shows a Sentinel-1 interferogram from April 20th, 2018 to May 2nd, 2018 over the Big Island of Hawaii. This time period spans the beginning of the 2018 Kilauea eruption event.
