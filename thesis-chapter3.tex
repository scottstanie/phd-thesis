...
InSAR time series techniques (sometimes called ``multi-temporal InSAR'' or MT-InSAR) are a class of methods for extracting information about the temporal evolution of surface deformation or other geophysical parameters of interest.  Most multi-temporal methods have generally been classified as belonging to one of the following categories: 1) stacking \citep{Zebker1997AtmosphericEffectsInterferometric, Sandwell1998PhaseGradientApproach}, 2) small-baseline approach \citep{Berardino2002NewAlgorithmSurface}, or 3) persistent-scatterer (PS) \citep{Ferretti2001PermanentScatterersSar, Hooper2006PersistentScatterRadar}. In the past decade, a fourth category of ``phase-linking`` approaches have been based on analysis of the complex covariance matrix of SAR image stacks \citep{Guarnieri2008ExploitationTargetStatistics, Ferretti2011NewAlgorithmProcessing, Ansari2018EfficientHighPrecision}. In the following section, we focus on (1) and (2).

%only be determined with a series of interferograms from SAR acquisitions over time,
