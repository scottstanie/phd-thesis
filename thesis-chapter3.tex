...
Below we illustrate correction attempts for two Sentinel-1 (path 78) interferograms over West Texas using GAM predictions (Figures \ref{fig:ch3-tropo-correct-wave}, \ref{fig:ch3-tropo-correct-storm}). 
...
For example, suppose an earthquake occurred at a known date, and a set of $M$ interferograms are formed from $N$ SAR acquisitions (where $ N/2 \leq M \leq (N + 1)N / 2 $) which all span the earthquake. To estimate the coseismic deformation $\theta$ at each pixel of the interferograms, the simplest stacking solution is
\begin{equation}
	\theta = \frac{\lambda }{4 \pi} \frac{1}{M} \sum_i^M \Delta \phi_i ,
	\label{eq:ch3-stacking-1}
\end{equation}
where $\Delta \phi_i$ is phase the $i^{th}$ interferogram, and the factor $  \frac{\lambda }{4 \pi} $ converts the phase from radians to centimeters.
...
%	v_{avg} = \frac{\lambda }{4 \pi} \frac{\sum_{i \in G} d_i}{\sum_{i \in G} t_i}
	v_{avg} = \frac{\lambda }{4 \pi} \frac{\sum_i \Delta \phi_i}{\sum_i t_i}
	\label{eq:ch3-stacking-2}
\end{equation}
where $\Delta \phi_i$ is phase the $i^{th}$ interferogram and $ t_i $ is the temporal baseline (i.e. time span between SAR acquisitions) of the interferogram.
%where $G$ is a subset of interferograms that were formed using two SAR scenes acquired within the time period.
% The LOS measurement (in cm) and the temporal baseline of the $i^{th}$ interferogram in $G$ are written as $d_i$ and $ t_i $ respectively
...
Given a set of M interferograms that have been unnwrapped and zero-referenced to a common location, the measurement vector
\begin{equation}
	\bm{\Delta \phi} = \left[ \Delta \phi_1 \ldots \Delta \phi_M \right]^T
\end{equation}
consists of the $M$ interferogram measurements for one ground pixel location. 
The goal is to solve for the unknown phase values $ \bm{\phi} $ which are contain the total LOS delay for each SAR acquisition. Note that each element $\phi_i$ consists of all elements of Equation \eqref{eq:ch3-insar-noise-terms} which affect the phase of a SAR acquisition (as opposed to individual interferogram noise terms); this includes the deformation phase, $\frac{4 \pi}{\lambda} \Delta r$, as well as the atmospheric delay terms, $\alpha_i$.
%The $\iota$
% unknown phase values associated with the deformation (AND DELAY! refer to the noise equationn)
%Accordingly, the expression (6) defines a system of equations in unknowns that may be organized in the following matrix representation
...
%In practical cases, as for the European Remote Sensing (ERS) satellites ERS-1 and ERS-2 sensors, the available acquisitions are generally distributed in small baseline (SB) subsets, separated by large “baselines”; accordingly, only one deformation time series for each subset can be easily obtained... ..the limitation to exploit all the available acquisitions for the generation of an overall deformation time-series is evident
%...demonstrated that small man-made features remain very correlated over time [14], [15], although interferograms over these areas only show pointwise information.
...
% REFER TO CHAPTER 5 at this line
