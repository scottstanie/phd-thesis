...
GACOS derives its corrections using the High Resolution ECMWF weather model (0.125-degree, 6-hour resolutions) and the GNSS derived tropospheric delay products from Neveda Geodetic Laboratory.
Note that for the West Texas region, GACOS mostly relies on the GAM inputs due to the sparsity of GPS stations.
...
We also compare the weather conditions that are visible at the secondary SAR acquisition time \footnote{For both Figures \ref{fig:ch2-tropo-correct-wave}, \ref{fig:ch2-tropo-correct-storm}, the earlier SAR acquisition occurs during winter, which is after sunset for Sentinel-1 path 78 over West Texas. Most of the tropospheric noise in these winter-to-summer interferograms comes from the summer SAR acquisition.} using the Geostationary Operational Environmental Satellites (GOES).
