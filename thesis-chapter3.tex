...
For topography mapping, two SAR antennas are separated by a distance in the across-track direction and each acquires a SAR image images either simultaneously or at different times under the assumption that no ground deformation has occurred (Figure \ref{fig:ch3-insar-geometry-topo}). The baseline causes the two radars to view the same point on the ground with a slight change in angle.
...
Since the measurement uses the phase of the propagated radar waves, it is only known modulo $0$ and $2\pi$; the process of obtaining the full continuous phase is known as \emph{phase unwrapping}
\citep{Goldstein1988SatelliteRadarInterferometry, Chen2001TwoDimensionalPhase}. Note that even after phase unwrapping, the absolute number of $2\pi$ cycles from the radar to the ground is not known; thus, an unwrapped interferogram represents the surface deformation relative to some point which is assumed to be constant.
...
%thermal noise
%
After removing phase contributions from topography and satellite geometry, the phase each pixel in an interferogram can be written as:
...
\begin{equation}
	\Delta \phi = \frac{4 \pi}{\lambda} \Delta_r +  \Delta \phi_{orb} + \Delta \phi_{dem} + \Delta \phi_{decor} + \Delta \phi_{unwrap} + \Delta \phi_{iono} + \Delta \phi_{tropo}  + \Delta \phi_{n}  
...
%The noise terms are summarized in Table \ref{tab:ch3-noises}.
The first two terms, $ \Delta \phi_{orb} $ and $ \Delta \phi_{dem} $, represent systematic errors resulting from imprecise knowledge of the platform's orbital position or topographic height. Orbital errors usually appear as a planar phase ramp in stripmap acquisitions, and were commonly encountered for ERS-1 or ENVISAT data; however they rarely appear in Sentinel-1 data due to it's high precision orbit determination \citep{Fattahi2014InsarUncertaintyDue}. Errors in the DEM result in an incorrect topographic phase removal and create a spatial baseline-dependent phase error \citep{Fattahi2013DemErrorCorrection}. These errors are generally in Sentinel-1 data due to the tight control of the repeat orbit tube.
%Here the noise terms here are divided into systematic errors, $ \Delta \phi_{sys} $, atmospheric propagation effects, $ \Delta \phi_{sys} $, phase decorrelation noise, $ \Delta \phi_{decor} $, and other smaller residual noise terms including thermal noise, $ \Delta \phi_{n} $.
...
Phase errors can arise from variations in the index of refraction from due to heterogeneities in the ionosphere, $\Delta \phi_{iono}$ \citep{Gray2000InfluenceIonosphericElectron}, or the troposphere, $\Delta \phi_{tropo}$ \citep{Zebker1997AtmosphericEffectsInterferometric}. Since the ionosphere is a dispersive medium, the phase effects are dependent on radar frequency, 
%with the index of refraction varying as $ \propto \frac{1}{f^2} $ \citep{Meyer2006PotentialLowFrequency}. 
%This means that 
with the effect at C-band being approximately 1/16 the effect at L-band \citep{Liang2019IonosphericCorrectionInsar}. Additionally, the dependence on frequency enables mitigation techniques which split the radar spectrum into sub-bands to calculate phase corrections \cite{Rosen2010MeasurementMitigationIonosphere}.
Tropospheric noise, $\Delta \phi_{tropo}$, arises from changes to pressure, temperature, or water vapor content in the neutral atmosphere.
...
\begin{figure}[!htbp]
	...
	\caption[Sentinel-1 interferogram over Hawaii showing common noise sources, along with 2018 eruption deformation]{
		...
		(b) The dense fringes near the coast are caused by turbulent tropospheric noise (see Section \ref{sec:ch3-noise-tropo-turb}).
...
For example, the coastal region in Figure \ref{fig:ch3-hawaii-example}b contains dense fringes which could be mistaken as deformation, but in fact is due to turbulent tropospheric noise (further explained in Section \ref{sec:ch3-noise-tropo-turb}).
The large elevation changes on Mauna Loa and Mauna Kea in the center of the island lead to stratified tropospheric noise (see Section \ref{sec:ch3-noise-tropo-strat}) which creates a concentric ring pattern that is the same shape as a subsidence bowl (Figure \ref{fig:ch3-hawaii-example}c).
...
The conversion between two-way distance from ground to satellite and InSAR in Equation \eqref{eq:ch3-phase-range} assumes that the electromagnetic signals travel though a homogeneous medium with constant velocity. In reality, they travel through the spatially inhomogenous atmosphere with variable index of refraction $n$, where $n$ relates to the phase velocity $v$ by $n = c/v$ \citep{Zebker1997AtmosphericEffectsInterferometric, Hanssen2001RadarInterferometryData, Liu2012SatelliteRadarInterferometry}.  Since $n$ is always real and slightly greater than $1$ for Earth's neutral atmosphere, it is more common to describe fluctuations using the \emph{refractivity} $N = 10^{6}(n - 1)$, which is the additional refractive index beyond unity.
...
\end{equation}
where $ds$ is the incremental slant length along the radar line of sight. The excess delay adds an additional phase of $\phi = - \frac{4 \pi}{\lambda} D$ to an SLC acquired with the given atmospheric conditions.
...
The coefficients $ k_1, k_2^{'}, k_3 $ and $ k_4 $ are constants estimated from meteorological data, commonly taken to be $ k_1 = 77.6$ , $ k_2^{'} = 23.3 $, $ k_3= 3.75 \cdot 10^5 $, and $ k_4 = 1.45 $ from \cite{Smith1953ConstantsEquationAtmospheric} and \cite{Solheim1999PropagationDelaysInduced}.


Although absolute delays decrease at lower altitudes as the signal travels through more of the atmosphere, an interferogram always measures a \emph{difference} of delays from two SAR acquisitions, and hence difference of refractivity. This means that although the hydrostatic delay is of the order of a few meters and the wet delay is $ \sim10$s of centimeters, phase artifacts in interferograms are more commonly caused by the spatial variations of the wet component of refractivity \citep{Zebker1997AtmosphericEffectsInterferometric, Hanssen2001RadarInterferometryData}. Additionally, the liquid component can cause several centimeters of delay in the presence of tall cumulonimbus thunder clouds which have a small ($< 10$km) spatial extent \citep{Liu2012SatelliteRadarInterferometry}.
...
For the purposes of InSAR analysis, noise from tropospheric delay variations is usually divided into a stratified component, which correlates with topography \citep{Doin2009CorrectionsStratifiedTropospheric}, and a turbulent component that is random at time scales longer than a day \citep{Emardson2003NeutralAtmosphericDelay, Onn2006ModelingWaterVapor}. 
...
\subsection{Mitigation of Tropospheric Noise}
\label{sec:ch3-noise-tropo-miotigate}
...
%While these approaches provide a measure of uncertainty in the deformation solution, the uncertainties are given for each pixel (or each model parameter), rather than for each visible deformation feature.
