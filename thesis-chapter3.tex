...
The SAR imaging geometry is shown in Figure \ref{fig:ch3-sar-geometry}. A side-looking radar on a platform 
%at height $h$ 
...
\begin{figure}
	...
	\caption[Acquisition geometry of an imaging radar]{Acquisition geometry of an  imaging radar. A platform containing a radar instrument is moving 
%		with speed $v$ 
in the azimuth/along-track direction.
%		 at a height $h$.
...
The complex-valued radar image $I$ at pixel location $ (x, y) $ can be formed by summing over all pulses and compensating for a range-dependent phase term:
\begin{equation}
	I(x, y) = \sum_{k} s_k \left(t_k \right) e^{-j \frac{4 \pi}{\lambda} r_k(x, y)}
\end{equation}
...
Interferometric synthetic aperture radar (InSAR) refers to a broad class of applications that exploit the phase difference between two SAR images to obtain extra information beyond the 2D locations of points \citep{Bamler1998SyntheticApertureRadar}. The most common applications are generating topography maps \citep{Graham1974SyntheticInterferometerRadar, Zebker1986TopographicMappingInterferometric} and mapping surface deformation \citep{Goldstein1987InterferometricRadarMeasurement, Gabriel1989MappingSmallElevation, Li1990StudiesMultibaselineSpaceborne, Massonnet1993DisplacementFieldLanders, Rosen2000SyntheticApertureRadar}. For topography mapping, two SAR antennas separated by a distance in the across-track direction acquire images either simultaneously or at different times under the assumption that no ground deformation has occurred (Figure \ref{fig:ch3-insar-geometry-topo}). The baseline causes the two radars to view the same point on the ground with a slight change in angle, causing a difference in phase.
This phase difference, $\Delta \phi$, is related to a difference in the geometrical path length between the two radars: $\Delta \phi = \frac{-4 \pi}{\lambda}(r_1 - 2_2)$. A different measured phase occurs when viewing flat Earth (Figure \ref{fig:ch3-insar-geometry-topo}a) vs. topography (Figure \ref{fig:ch3-insar-geometry-topo}b). Thus, with precise knowledge of the viewing geometry, the phase can be converted to an altitude for each point in the SAR images \cite{Simons2007InterferometricSyntheticAperture}. 
...
\begin{figure}
	...
	\caption[InSAR imaging geometry for topography mapping]{InSAR imaging geometry for topography mapping.
		(a) Two SAR images, $S_1$ and $S_2$, are acquired with a slight separation, resulting in a different geometric path length to each ground point, $r_1$ and $r_2$.
	}
...
In repeat-pass InSAR (sometimes called \emph{differential} InSAR, or DInSAR), the same radar platform flies over an area and acquires a SAR images at two different times $t_1$ and $t_2$ (Figure \ref{fig:ch3-insar-geometry-defo}). Assuming that the topography is known and can be removed from the phase measurement, the InSAR phase measures range change,  $r_2 - r_1$ between the two times. In the repeat pass setup, a small spatial baseline between the satellite at the two times is desired, while the topography application is more sensitive when the baseline is larger.


\begin{equation}
	z_{1,2} = S_1 S_2^{*} = |S_1||S_2| e^{\phi_2 - \phi_1}
\end{equation}
where $S_k = |S_k| e^{-j 4 \pi / \lambda r_k}$
...
\begin{figure}
	\centering
	\includegraphics[width=0.99\linewidth]{figures/chapter3-sar/ch3-insar-geometry-defo.pdf}
%	\includegraphics[width=0.99\linewidth]{figures/chapter3-sar/ch3-insar-geometry-defo-2col.pdf}
	\caption[InSAR imaging geometry for deformation mapping]{InSAR imaging geometry for deformation mapping.
		(a) Two SAR images, $S_1$ and $S_2$, are acquired at different times, before and after a ground deformation event. The change in range between times $t_1$ and $t_2$ results in a measured phase change $\Delta \phi =  \frac{-4 \pi}{\lambda}(r_2 - r_1)$.
	}
