...
As described in Section \ref{sec:ch2-noise-tropo}, tropospheric noise $\Delta \phi_{tropo}$ consists of a stratified component that correlates with topography \citep{Doin2009CorrectionsStratifiedTropospheric} and a turbulent component that is random at time scales longer than one day \citep{Emardson2003NeutralAtmosphericDelay}.
...
We tested the Generic Atmospheric Correction Online Service (GACOS, \citep{Yu2018InterferometricSyntheticAperture,Yu2018GenericAtmosphericCorrection}), which combines global atmospheric models (GAMs) and GPS data to produce synthetic delay maps, as well as the line-of-sight path integration of GAMs using the ray tracing library of \cite{Maurer2021RaiderRaytracingAtmospheric}.
% tropospheric correction estimate. 
Due to the sparsity of GPS stations, the GACOS corrections for West Texas mostly rely on the GAM input, so the methods showed similar corrections.
% GAMs and GPS data . 
% stratified tropospheric noise component using 
%
% and we found that the stratified tropospheric errors in our Sentinel-1 data are minimal.
...
We illustrate two correction attempts for Sentinel-1 (path 78) interferograms over West Texas using GAM predictions (Figures \ref{fig:ch2-tropo-correct-storm}, \ref{fig:ch2-tropo-correct-wave}). 
The corrections shown here are produced by the Raytracing Atmospheric Delay Estimation for Radar (RAiDER) library \citep{Maurer2021RaiderRaytracingAtmospheric}, which generates a troposphere correction product from a user-selected GAM by integrating along the radar LOS paths. We use NASA's Global Modeling and Assimilation Office (GMAO) reanalysis weather models, which are interpolated by the RAiDER library to the same pixel spacing as the interferograms ($\sim$ 500 m for this example). We also compare the weather conditions that are visible at the secondary SAR acquisition time \footnote{For both Figures \ref{fig:ch2-tropo-correct-wave}, \ref{fig:ch2-tropo-correct-storm}, the earlier SAR acquisition occurs during winter, which is after sunset for Sentinel-1 path 78 over West Texas. Most of the tropospheric noise in these winter-to-summer interferograms comes from the summer SAR acquisition.} using the Geostationary Operational Environmental Satellites (GOES).

In the first example, the secondary acquisition of the interferogram contains a summer thunderstorm (Figure \ref{fig:ch2-tropo-correct-storm}a). The tall cumulonimbus clouds create patches of excess delay of $>10$ cm (Figure \ref{fig:ch2-tropo-correct-storm}b)\footnote{The different imaging geometries of the Sentinel-1 satellites in low Earth orbit and the GOES satellites in geostationary orbit mean the pixels of Figure \ref{fig:ch2-tropo-correct-storm}a and Figure \ref{fig:ch2-tropo-correct-storm}b do not fully align, but cover approximately the same area.}.
...
%Since the correction methods based on auxiliary data 
Since the turbulent tropospheric noise is more difficult to predict using the auxiliary correction methods, Chapters \ref{CHAP:4-GRL} and \ref{CHAP:5-robust-ts} present data-driven mitigation strategies for producing robust estimates of surface deformation.
% despite the strong tropospheric noise.
