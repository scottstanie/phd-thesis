...
The magnitude of this quantity $ \rho = |\gamma | $ is called the \emph{correlation} (or sometimes called the coherence), and ranges from $ 0 $, indicating a complete loss of signal coherence, to $ 1 $, indicating perfectly correlated radar echoes.
%Decorrealtion 
...
Note that although decorrelation is one of the most common noise sources for deformation mapping, the presence of decorrelation can be considered a signal (e.g. for coherent change detection \citep{Simons2007InterferometricSyntheticAperture, Jung2016CoherentChangeDetection}).

%TODO: Better intro sentence about index of refraction?
The propagation of radar waves through the atmospheric can produce errors due to the ionosphere, $\Delta \phi_{iono}$ \citep{Gray2000InfluenceIonosphericElectron}, or the troposphere, $\Delta \phi_{tropo}$ \citep{Zebker1997AtmosphericEffectsInterferometric}. Since the ionosphere is a dispersive  medium, the phase effects are dependent on radar frequency, 
%with the index of refraction varying as $ \propto \frac{1}{f^2} $ \citep{Meyer2006PotentialLowFrequency}. 
%This means that 
with the effect at C-band being approximately 1/16 the effect at L-band \citep{Liang2019IonosphericCorrectionInsar}. Additionally, the dependence on frequency enables mitigation techniques which split the radar spectrum into sub-bands to determine the ionospheric phase shifts \cite{Rosen2010MeasurementMitigationIonosphere}.
...
Since tropospheric noise is the dominant noise for the study area of this thesis, the noise is further detailed in Section \ref{sec:ch3-noise-tropo}.
...
In high signal to noise ratio (SNR) interferograms (e.g. for large magnitude earthquakes such as \cite{Massonnet1993DisplacementFieldLanders}), one can estimate the deformation magnitude by simply ``counting the fringes'' and multiplying by $\lambda/2$. However, it is more common for an interferogram to contain many visual features corresponding to noise terms in Equation \eqref{eq:noise-terms}. These noise sources can often be visually identified, but they can add considerable difficulty for inexperienced InSAR users.
...
To illustrate this, Figure \ref{fig:ch3-hawaii-example} shows a Sentinel-1 interferogram of the Big Island of Hawaii from April 20th, 2018 to May 2nd, 2018. 
Because much of Hawaii is a tropical island (?), there are large changes to the tropospheric water vapor content from, leading to turbulent tropospheric noise (Figure \ref{fig:ch3-hawaii-example}b). 
Additionally, the 
This time period spans the beginning of the 2018 Kilauea eruption event.

The windward side of the island contains many dense tropical rain forests (Figure \ref{fig:ch3-hawaii-example}e). These areas show heavy decorrelation at the Sentinel-1 C-band wavelength of 5.5 cm (Figure \ref{fig:ch3-hawaii-example}d).
