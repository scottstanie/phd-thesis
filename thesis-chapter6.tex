...
We tested our deformation detection algorithm using the three ascending and three descending cumulative surface deformation maps derived from the stacking method outlined in Chapter \ref{CHAP:4-GRL}. For each SAR acquisition date, we estimated the tropospheric turbulence noise using all interferograms that contain the SAR scene based on Equation \eqref{eq:avg-ifg}. We removed a quadratic ramp in each noise map, and we calculated the average 1D PSD of all 84 noise maps as described in \ref{sec:ch6-methods-2-tropo-spectrum}.
...
We similarly characterized the tropospheric noise for the descending Path 85 data. We then identified features that are likely real in the three cumulative deformation maps spanning November 2014 to January 2017, January 2018, and January 2019.
