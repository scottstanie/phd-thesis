...
A rigorous analysis of all noise sources in the Permian Basin Sentinel-1 InSAR data identified tropospheric turbulence noise as the dominant noise term.
We developed methods for characterizing the tropospheric noise and its power spectral density directly from InSAR data, and we designed scalable, robust time series algorithms for reconstructing the temporal evolution of surface deformation over very wide regions which are robust to troposphere noise outliers.
Based on independent validation from GPS permanent stations, our cumulative surface deformation solutions achieved millimeter-level accuracy.
%, as well as methods for mitigating the impact of the troposphere noise outliers.
...
We developed a computer vision algorithm capable of detecting deformation features of unknown size and location in large deformation maps.
Combined with our tropospheric noise characterization, we produced uncertainty measures for each automatically detected deformation feature and observed a growing number of high-confidence detected deformation features as oil production rose in the Basin.
