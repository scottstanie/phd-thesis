...
A rigorous analysis of all noise sources in the Permian Basin Sentinel-1 InSAR data identified tropospheric turbulence noise as the dominant noise term, and we developed methods for characterizing the tropospheric noise and its power spectral density directly from InSAR data.
We designed scalable time series algorithms which are robust to troposphere noise outliers for reconstructing the temporal evolution of surface deformation over very wide regions.
Based on independent validation from GPS permanent stations, our cumulative surface deformation solutions from 2014 - 2021 achieved millimeter-level accuracy.
%, as well as methods for mitigating the impact of the troposphere noise outliers.
Furthermore, the recent growth in satellite data has created a need for automatically locating deformation signal of interest.
...
We developed a computer vision algorithm capable of detecting deformation features of unknown size and location in large deformation maps. Combined with our tropospheric noise characterization, our algorithm produces uncertainty measures for each automatically detected deformation feature. We detected a growing number of high-confidence features as oil production rose across the basin, including numerous subsidence and uplift features near active production and disposal wells and linear deformation patterns associated with fault activities near clusters of earthquakes.
