...
%Certain induced earthquakes have occurred 10s of kilometers from the location of the injection well after the fluid migrated in the subsurface.
...
%developing methods to produce reliable surface deformation maps over large regions, despite the presence of strong atmospheric noise. We have also focused on creating methods for quantifying uncertainty which are easy to interpret for non-expert stakeholders.

%- goal : Design scalable insar algorithms to map defo over west texas with robust uncertainty quanty so non insar expert can use for their interpretation
...
%	\includegraphics[width=\textwidth]{}
	\caption{Line of sight convention used in this thesis for ascending (a) and descending (b) satellite geometries. Vector points from the satellite to the ground}
...
\label{sec:methods}
%\subsection{Overview}
%\textcolor{red}{We need to summarize three subsections in a paragraph.}

%\textcolor{red}{[We will discuss Section 1 and 2.A on Wedenesday. It is awkward to say "initial goal". What is the final goal? I think your algorithm has two major component (A and B/C). An important thing to note is A alone cannot be used to detect deformation.]}.The initial goal of our algorithm is to automatically detect all surface deformation signals in InSAR maps. We approach this using a filter-based computer vision algorithm that can search over many image scales to find spatially coherent image features.  \textcolor{red}{[Maybe avoid using "false alarm", Most of times, it is not a big deal for the surface to move a few cm]}
%Second, we wish to calculate a probability of false positive for each detection. Since only the distribution of noise is required to calculate a false positive probability, and not the distribution of the signals of interest, we incorporate a careful tropospheric noise analysis of each SAR acquisition over our study area. This allows is to simulate realistic tropospheric turbulence noise and determine the frequency that certain artifacts of a given size and magnitude could be detected by the computer vision algorithm. We then keep only the candidate deformation detections in our InSAR maps which have very low probability of being tropospheric noise artifacts.
% \textcolor{red}{The transition between the two components is not very smooth. The way it currently reads like we developed an algorithm to detect deformation signals. However, it actually does not work, because there is noise. It is rather confusing because to InSAR expert tropospheric noise is not the only noise source and we are single-mined on that. Those designs need to be articulated carefully. The two components are rather decoupled.}

%\textcolor{red}{An overview of the algorithm is not a list of what you did, but the overall ideas behind the algorithm design. The first part of the algorithm is to detect blob-like features, which can be signal and noise. And the second part is you figure out a way to UQ and remove the noise blobs. I think the way you describe how the computer vision algorithm works is great. We need to figure out how to talk about the noise estimation without a list of what we did. Also, it is a good idea to finish talk about part 1, before move to part 2.}
...
\begin{figure}[hbt!]
	\centering 
	\includegraphics[width=0.9\linewidth]{paper2/figures/figure5_noise_path78.png}
...
\begin{figure}[hbt!]
	...
	\includegraphics[width=0.9\linewidth]{paper2/figures/figure_results_kde.pdf}
