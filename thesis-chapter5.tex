...
Results of smoothing the synthetic noisy time series $ \bm{\phi} $ (blue with dots) using the LOWESS algorithm (green) and a triangle filter (pink). True deformation time series is shown in orange.
 Both the LOWESS algorithm and the triangle filter use $ 1/3 $ of the data (a filter width of 1 year) and have been shifted to attempt to remove the first day's atmospheric noise.

	}
	\label{fig:ch5-compare-tri}
\end{figure}

After subtracting $ \phi_0 $ from all points, the results of the LOWESS estimate $ \bm{\hat{\phi}} $ are shown in Figure \ref{fig:ch5-compare-tri} (green line). The LOWESS algorithm smooths away the large spikes of turbulence noise in  $ \bm{\phi} $. We note that since the width of the weighting window $ \bm{w} $ was 1 year, the algorithm smooths over the peak of the deformation.
For comparison, we have also smoothed the noisy time series with a 1-year wide triangle filter (Figure \ref{fig:ch5-compare-tri}, pink line). The triangle filter performs similarly if the noise is Gaussian; however, in this example with several large turbulence jumps, the triangle filter is pulled 5-10 mm away from the real deformation time series (Figure \ref{fig:ch5-compare-tri}, orange line).
