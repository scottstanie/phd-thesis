...
The true deformation simulates linearly uplift which linearly increases to $ \sim3 $cm max after 1.5 years and deflates back to zero by the end of 3 years. 
Turbulent tropospheric noise is simulated for each date following \cite{Hanssen2001RadarInterferometryData} added to the deformation.
...
For comparison, we have also smoothed the noisy time series with a 1.2 year wide triangle filter (Figure \ref{fig:ch5-compare-tri} pink line). 
...
We processed an additional three years of Sentinel-1 data (January 2019 though December, 2021) for Path 78 (Figure \ref{fig:paper1-study-area}) following the processing strategy outlined in Section \ref{sec:ch3-insar-processing}, for a total of 151 SAR acquisitions from Nov. 2014 to Dec. 2021
We performed an unregularized SBAS inversion to get a noisy time series $ \bm{\phi} $ for each pixel. We fit and removed a quadratic surface from the deformation image at each SAR time step to mitigate residual long wavelength noise from seasonal tropospheric patterns \citep{Morishita2020LicsbasOpenSource}. We temporally smoothed each pixel's time series with the LOWESS algorithm using a weighting window covering $\geq 2$ years at all time intervals  and 2 robust iterations. We similarly processed 147 descending Path 85 SAR acquisitions and created a 7-year cumulative LOS deformation time series.
...
instead, we used the referencing technique presented in \cite{Zebker2021AccuracyModelFree} to fit and remove a phase-elevation trend from all high-coherence pixels in each interferogram and zero-reference the deformation. This method works well for interferograms covering large regions where only a subset of high-coherence pixels contain significant deformation.
...
The cumulative LOS deformation from Nov. 2014 to Dec. 2021 is shown in Figure \ref{fig:ch5-results-los} for Path 151 (panel (a)), Path 85 (panel (b)), and Path 78 (panel (c)). Areas with low average spatial coherence ($ < 0.2 $) or temporal coherence $ < 0.75 $ are masked. In the northern Delaware Basin, many of the subsidence patterns present in the 4-year cumulative maps (Figure \ref{fig:ch4-insar-los}) show an acceleration as oil production 
...
\begin{figure}
	...
	\caption[Path 78 deformation features]{
		...
		Grey dot shows the location of Texas salt water disposal well (API  49533675).
		(b) Time series of pixel indicated by grey dot in panel (a) showing noisy $ \bm{\phi} $ (blue) and LOWESS result (orange). Green bars show the annual injection volume, indicating that the well shut down injection operations in 2018.
