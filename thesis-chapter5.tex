...
Consider a time series of $N$ LOS phase delays for a single pixel,  $ \bm{\phi} = \left[\phi_0, \phi_1, \ldots, \phi_{N-1} \right]^T $, solved from the SBAS linear system (Equation \eqref{eq:ch2-sbas-A}).
Here we consider the case that the interferogram-specific noise sources (e.g. $  \Delta \phi_{decor}, \Delta \phi_{unwrap}, \Delta \phi_{scat}  $) have been properly accounted for in the inversion for $ \bm{\phi} $ so that the noise from $ \alpha_i $ is much larger than other residual noises.
...
\end{equation}
where $ d_i$ and  $\alpha_i$ are the (relative )surface deformation and atmospheric delay on the $i$th day, and $ \lambda $ is the radar wavelength.
%Since deformation is usually the quantity of interest,
Our goal is to separate the vector of surface deformation $ \bm{d} =  \left[d_0, d_1, \ldots, d_{N-1} \right]^T $ from the atmospheric noise contribution $ \bm{\alpha} $.
...
The most widely used non-parametric methods for separating $ \bm{d} $ from $ \bm{\alpha} $ rely on the fact that the turbulent component of $\bm{\alpha}$ is uncorrelated in time (at the repeat interval of SAR acquisition), while most deformation sources show strong temporal correlation.
...
Once difficulty with the triangular filter and other moving-average filters is that the filter coefficients are non-adaptive, and noisier atmospheric conditions will be weighted the same as calmer conditions \citep{Liu2012SatelliteRadarInterferometry}.
...
We can adaptively filter the noisy $ \bm{\phi} $ time series by implementing a robust LOWESS regression. The LOWESS algorithm moves along the time axis and fits a local linear regression to nearby data points, repeating over multiple iterations to refine the fit and reduce the effect of outliers \citep{Efron2019ComputerAgeStatistical}.
% which performs several iterations of weighted linear regression for each SAR acquisition. 
For the first iteration at time $t_i$,  we calculate a set of weights which is centered around $t_i$. The weight function has with a maximum value of $1$ at $t_i$ and decays to zero at the $r$th nearest nneighbor to $t_i$, where $r = \left\lfloor \gamma N \right\rfloor$ and $0 < \gamma \leq 1$ is the fraction of the data chosen to include for each local fit.
Lett $h_i$ be the $r$th smallest number of $\left| x_i - x_k \right|$ for $ k = 0, \ldots, N-1 $.
The weight $w_k(t_i)$ for the $k$th data point, defined as
\begin{equation}
	w_k(t_i) = 
	\begin{cases}
...
\end{equation}
for $k =0, \ldots, N-1 $, is known as the \emph{tricube} weighting function.
Figure \ref{fig:ch5-algo-window} shows three example $ \bm{w}(t_i) $ weights for $\gamma = 1/6, 1/3, 1/2 $.
A synthetic time series $\phi$ (gold dots) spans 3 years and has with a data point every 12 days, making the three $ \gamma $ choices equal to $0.5, 1,$ and $1.5$ years.

\begin{figure}
	\centering
	\includegraphics[width=.99\textwidth]{figures/chapter5-lowess/figure2-fits.pdf}
	\caption[Demo of LOWESS fitting]{
		(a) First iteration of local fit at $t_i$, weighted by window (orange). Shading of dots indicates zero weight (gray) up to full weighting (gold).
		(b) First iteration of smoothed $ \hat{\phi}_i $ for all $i$. 
		(c) Second iteration at $t_i$, where the weighting combines the original window with the updated residual weighting. The new local fit (green slope) has downweighted the effect of the two large values compared to the original (orange slope).
		% to produce a new weighted least squares fit.
		(d) Full result of second iteration of LOWESS smoothing (green).
	}
...
\end{equation}
The smoothed estimate $\hat{\phi}_i$ at the point $t_i$ is calculated as $\hat{\phi}_i = a_i + t_i b_i$. This process is repeated at each time $t_i$ for $i = 0, \ldots, N-1$.

The second iteration performs another weighted least squares fit with an additional downweighting based on the residuals of the first iteration.
...
\end{equation}
Similar to the use of the median absolute deviation from Section \ref{sec:ch4-outlier-method}, Equation \eqref{eq:ch5-lowess-weights2} uses the median $ s $ so that large outliers are clipped to 0. For each new fit in the second iteration, the weights $ \bm{w}(t_i) $ are multiplied by the residual weights $\bm{delta}$ to perform the local regressions.

Figure \ref{fig:ch5-algo-demo} illustrates two iterations of iterations 
