...
This step is necessary for $ \bm{\phi} $ solved using the SBAS formulation of Equation \eqref{eq:ch2-sbas-A}, which assumes the first date's phase is equal to 0 and causes $ \alpha_0 $ to be present in all dates.
...
We first illustrate the LOWESS algorithm on a synthetic 3-year deformation time series with a 12-day acquistion interval (Figure \ref{fig:ch5-demo-data}).
A 300 x 300 pixel region contains an uplift bowl which linearly increases to $ 3 $cm max after 1.5 years (Figure \ref{fig:ch5-demo-data}a) and deflates back to zero by the end of 3 years. Turbulent tropospheric noise is simulated for each date following \cite{Hanssen2001RadarInterferometryData} added to the deformation (Figure \ref{fig:ch5-demo-data}b). The uplift bowl inflates at a constant rate for 1.5 years, then deflates back to 0 by the end of the 3rd year (Figure \ref{fig:ch5-demo-data}c).
...
The second iteration is influenced less by the outlier points and has fewer spurious bumps in the time series (Figure \ref{fig:ch5-algo-demo}d). Note that more iterations can be performed if desired; in this example, further iterations provide little benefit ($ \sim $ 1 mm or less).
...
After subtracting $ \phi_0 $ from all points, the results of the LOWESS estimate $ \bm{\hat{\phi}} $ are shown in Figure \ref{fig:ch5-compare-tri}a (green line). The LOWESS algorithm smooths away the large spikes of turbulence noise in  $ \bm{\phi} $ Figure \ref{fig:ch5-compare-tri}a (blue). 
...
For comparison, we have also smoothed the noisy time series with a 1.2 year wide triangle filter (Figure \ref{fig:ch5-compare-tri}a pink line). 
The triangle filter performs similarly to LOWESS when the noise is Gaussian; however, in this example with several large turbulence jumps, the triangle filter is pulled $>2$ cm away from the true deformation time series (Figure \ref{fig:ch5-compare-tri}d).
...
We processed an additional three years of Sentinel-1 data (January 2019 though December, 2021) for Path 78 (Figure \ref{fig:paper1-study-area}) following the processing strategy outlined in Section \ref{sec:ch3-insar-processing}. 
We performed an unregularized SBAS inversion to get a noisy time series $ \bm{\phi} $ for each pixel on 151 SAR acquisitions from Nov. 2014 to Dec. 2021. We fit and removed a quadratic surface at each noisy deformation time step to mitigate residual long wavelength noise from seasonal tropospheric patterns. We temporally smoothed each pixel's time series with the LOWESS algorithm using a weighting window covering $\geq 2$ years at all time intervals  and 2 robust iterations. We similarly processed 147 descending Path 85 SAR acquisitions and created a 7-year cumulative LOS deformation time series.
...
The Path 151 footprint does not contain GPS station TXKM that was used as the reference location;
%therefore, or any other GPS station near the center of the footprint, 
for this dataset, we used the referencing technique presented in \cite{Zebker2021AccuracyModelFree} to fit and remove a phase-elevation trend from all high-coherence pixels in each interferogram. This method relies on the assumption that the majority of pixels in all interferogram do not show significant deformation, which is a safe assumption for Path 151 which only partially overlaps with the Permian Basin's production region.
...
Figure \ref{fig:ch5-results-gps-example} shows two example GPS stations TXFS (Figure \ref{fig:ch5-results-gps-example}a) and station TXSO (Figure \ref{fig:ch5-results-gps-example}b).
The LOWESS averaged solutions are unaffected by the
The unregularized InSAR time series (green x's) contain 5-7 cm spikes of tropospheric noise. However, the LOWESS solutions (thick green lines) are unaffected by the spikes and successfully track the $ \sim1 $cm of long term motion shown in the GPS data (orange line). Note that seasonal variations found in the TXSO GPS data are not captured in the LOWESS solution due to the $2$ year local weighting window.
%to compare the LOWESS smoothed cumulative deformation results from Nov. 2014 to Dec. 2021.
...
The additional 3 years of Sentinel-1 data means there are an additional two stations (TXM5, TXB8) with sufficient data coverage for comparison, for a total of 15 stations within the Path 78 footprint.
The average RMS difference across the 15 stations between the smoothed GPS time series and the InSAR LOWESS solutions is 5 mm, and the maximum absolute difference at any time point, including the seasonal variations, for all stations is 1.5 cm.
...
%	\includegraphics[width=.99\textwidth]{figures/chapter5-lowess/gps_path78_2.pdf}
	\caption[Full GPS comparison for Path 78 LOWESS solutions]{
		Comparison of Path 78 LOWESS smoothed algorithm (green lines) with permanent GPS stations projected onto the radar LOS (orange dots). Orange line shows 30 day moving average of GPS daily measurements.
	}
...
We merged the two vertical (and eastward) deformation maps in their region of overlap to obtain one continuous vertical (eastward) map (Figure \ref{fig:ch5-results-east-up}).
