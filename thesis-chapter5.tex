...
The LOWESS smoothing technique easily accounts for nonlinear deformation while also suppressing the strong summer tropospheric noise (10-15 cm).
We demonstrate the technique on a synthetic time series and on three paths of Sentinel-1 data over the Permian Basin.
...
%	\includegraphics[width=.99\textwidth]{figures/chapter5-lowess/gps_path78_2.pdf}
	\caption[Full GPS comparison for Path 78 LOWESS solutions]{
		Comparison of Path 78 LOWESS smoothed algorithm (orange lines) with daily permanent GPS stations projected onto the radar LOS (blue dots). Orange lines show 30 day moving average of GPS daily measurements.
	}
...
\subsection{Choosing the LOWESS window size}

Tradeoff between noise reduction and temporal resolution.

We compared multiple $ \gamma $ choices...

As a measure of uncertainty, we usee the bootstrap technique \citep{Efron2019ComputerAgeStatistical},  \citep{Efron1979BootstrapMethodsAnother}
