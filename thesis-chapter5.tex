...
Results of smoothing the synthetic noisy time series $ \bm{\phi} $ (blue with dots) using the LOWESS algorithm (green) and a triangle filter (pink). Both the LOWESS algorithm and the triangle filter use $ 1/3 $ of the data (a filter width of 1 year) and have been shifted to attempt to remove the first day's atmospheric noise.
	}
	\label{fig:ch5-compare-tri}
\end{figure}
